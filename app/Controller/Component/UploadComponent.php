...
class UploadComponent extends Component {
    ...
    // New construct which implements Cakephp's component constructor
    ...
    public function __construct(ComponentCollection $collection, $options = null) {
      ...
      // Checks if new options are passsed in
      if ($options) {
          $this->options = $options + $this->options;
      }

      // Checks if error messages are passed in
      if (isset($this->options['error_messages'])) {
          $this->error_messages = $this->options['error_messages'] + $this->error_messages;
          unset($this->options['error_messages']);
      }

      // Calls component constructor
      parent::__construct($collection, $this->options);
    }
    ...
    protected function upcount_name($name) {
        return preg_replace_callback(
            '/(?:(?: \(([\d]+)\))?(\.[^.]+))?$/', //regexp
            array($this, 'upcount_name_callback'), // replacing callback
            $name, // string where to search and replace
            1 //line
        );
    }
