...
// Creates new istance of genome config validator
...
$(function() {
  ...
  $fields.filter('[id^=\'config-file-\']').each(function(){
    ...
    $.ajax({
      url: Defaults.apiRoot + 'defaults/' + file,
      method: 'GET'
    }).done(function(data){
      if(!editor.getValue()) {
  ...
  $.ajax({
    url: Defaults.apiRoot + 'genome_configs/' + GenomeConfig.id,
    method: 'GET',
    dataType: 'json'
  })
  .done(function(data){
  ...
  .always(function(data){
    ...
    loadGenomeConfig(data);

    // If GenomeConfig is set, triggers validation
    if(GenomeConfig.id != undefined) {
...
function saveConfigSession(data) {
  ...
  return $.ajax({
    url: Defaults.apiRoot + 'genome_configs/' + (GenomeConfig.id ? GenomeConfig.id : ''),
    method: 'POST',
    processData: false,
    contentType: false,
    data: data,
...
function validate(field, value, callback) {
 ...
 callback(validator.validate(field, value));
}
...
function validateSpeciesName(value, callback, delay = 0) {
 ...
 validate('species_name', value, function(result) {
   ...
   // Checks session type
   if(!GenomeConfig || GenomeConfig.type != 'update') {
     // Executes callback on local validation result
     callback(result);
     // Exits execution flow
     return delayed;
...
function clickSaveButton(){
 ...
 validateConfigSession(function(result){
   ...
   // Serializes data
   var data = serializeGenomeConfig();
   // Saves data
